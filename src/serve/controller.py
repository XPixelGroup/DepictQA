...
from .utils import CONTROLLER_HEART_BEAT_EXPIRATION, build_logger, server_error_msg
...
class Controller:
    ...
    def get_worker_status(self, worker_name: str):
        try:
            r = requests.post(worker_name + "/worker_get_status", timeout=30)
...
if __name__ == "__main__":
    ...
    cfg.controller["log_dir"] = cfg["log_dir"]
    cfg = cfg["controller"]

    os.makedirs(cfg["log_dir"], exist_ok=True)
    logger = build_logger("controller", os.path.join(cfg["log_dir"], "controller.log"))
    logger.info("cfg: {}".format(pprint.pformat(cfg)))

    controller = Controller(cfg["dispatch_method"])
